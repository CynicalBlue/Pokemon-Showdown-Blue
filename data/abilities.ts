...
export const Abilities: {[abilityid: string]: AbilityData} = {
	...
	natureadept: {
		...
		onTryHit(target, source, move) {
			if (target !== source && move.type === 'Bug') {
				this.add('-immune', target, '[from] ability: Nature Adept');
				return null;
			}
	...
	reinforced: {
		onTryHit(pokemon, target, move) {
			if (move.flags['pulse']) {
				this.add('-immune', pokemon, '[from] ability: Reinforced');
				return null;
			}
		},
		isBreakable: true,
	...
	stall: {
		onStart(pokemon) {
			this.add('-ability', pokemon, 'Stall');
		},
		onDeductPP(target, source) {
			if (target.isAlly(source)) return;
			return 2;
