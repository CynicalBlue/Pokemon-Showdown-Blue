export const Items: {[itemid: string]: ItemData} = {
	...
	roomcheckout: {
		...
		onStart(pokemon) {
			if (!pokemon.ignoringItem() && this.field.getPseudoWeather('trickroom' || 'magicroom' || 'wonderroom')) {
				if (pokemon.hp) {
					...
					if (pokemon.useItem()) pokemon.switchFlag = true;
				}		
			}
		},
		onAnyPseudoWeatherChange() {
			const pokemon = this.effectState.target;
			if (this.field.getPseudoWeather('trickroom' || 'magicroom' || 'wonderroom')) {
				if (pokemon.hp) {
					if (!this.canSwitch(pokemon.side)) return;
					if (pokemon.volatiles['commanding'] || pokemon.volatiles['commanded']) return;
					for (const pokemon of this.getAllActive()) {
	...
	roomservice: {
		...
		onStart(pokemon) {
			if (!pokemon.ignoringItem() && this.field.getPseudoWeather('wonderroom')) {
				pokemon.useItem();
			}
		},
		onAnyPseudoWeatherChange() {
