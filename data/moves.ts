...
export const Moves: {[moveid: string]: MoveData} = {
	...
	ingrain: {
		...
		onTry(source, target, move) {
			if (source.volatiles['ingrain']) return false;
	...
	magicroom: {
		...
		condition: {
			...
			durationCallback(source, effect) {
				...
				if (source?.hasItem('roomupkeep')) {
					return 8;
				}
				if (source?.hasAbility('arcane')) {
					return 5;
				}
				return 5;
			},
	...
	slurp: {
		...
		flags: {protect: 1, reflectable: 1, mirror: 1},
		onHit(target, source, move) {
			if (target.volatiles['disable']) return;
	...
	sonicpunch: {
		...
		pp: 30,
		priority: 1,
		flags: {contact: 1, protect: 1, mirror: 1, punch: 1, sound: 1, bypasssub: 1},
		secondary: null,
		target: "normal",
		type: "Sound",
		contestType: "Cool",
	...
	trickroom: {
		...
		condition: {
			...
			durationCallback(source, effect) {
				...
				if (source?.hasAbility('ancienttrickery')) {
				return 5;
				}
				return 5;
			},
	...
	wonderroom: {
		...
		condition: {
			...
			durationCallback(source, effect) {
				...
				if (source?.hasAbility('marvelous')) {
					return 5;
				}
				return 5;
			},
